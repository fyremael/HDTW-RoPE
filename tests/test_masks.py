...
def test_padding_receives_no_attention_probability() -> None:
    frequency = ClockFrequencyMap(
        num_heads=2, rotary_dim=4, component_names=["position"], mode="fixed_log"
    )
    module = HDTWRoPECrossAttention(
        model_dim=16, num_heads=2, rotary=ClockRotaryEmbedding(frequency)
    )
