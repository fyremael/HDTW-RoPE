...
def test_noiseless_synthetic_clock_recovery(family: str) -> None:
    pair = generate_synthetic_pair(
        audio_length=80, lyric_length=60, feature_dim=48, warp_family=family, noise_std=0.0, seed=7
    ...
    clock = LatentClockExtractor(["aligned_word"])(
        {"aligned_word": alignment},
        {},
        {"aligned_word": torch.linspace(0.0, 1.0, target.shape[1]).unsqueeze(0)},
        {},
...
def test_moderate_noise_synthetic_clock_recovery() -> None:
    ...
    target_mask = pair.sample.lyric_mask.unsqueeze(0)
    alignment = hard_dtw(
        PairwiseCost("cosine")(source, target, source_mask, target_mask), source_mask, target_mask
    ).alignment
    clock = LatentClockExtractor(["aligned_word"])(
        {"aligned_word": alignment},
        {},
