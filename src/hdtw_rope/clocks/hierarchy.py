...
def containment_loss(
    ...
    if not (child_clock.shape == parent_lower.shape == parent_upper.shape == valid_mask.shape):
    ...
    violation = torch.relu(parent_lower - child_clock) + torch.relu(child_clock - parent_upper)
...
def containment_violation_rate(
    ...
    *,
    ...
    violation = (
        (child_clock < parent_lower - tolerance) | (child_clock > parent_upper + tolerance)
    ) & valid_mask
