...
def word_onset_metrics(
    predicted_seconds: Tensor,
    target_seconds: Tensor,
    mask: Tensor,
    *,
    tolerances: Sequence[float] = (0.1, 0.3, 0.5, 1.0),
) -> AlignmentMetrics:
    ...
    return AlignmentMetrics(
        mean_absolute_error_seconds=float(error.mean().item()),
        median_absolute_error_seconds=float(error.median().item()),
        within_tolerance={
            float(tol): float((error <= tol).float().mean().item()) for tol in tolerances
        },
    )
...
def boundary_f1(predicted: Tensor, target: Tensor, mask: Tensor, *, tolerance: float) -> float:
    ...
    for value in predicted_values:
        candidates = [
            (abs(value - target_value), index)
            for index, target_value in enumerate(target_values)
            if index not in matched_target and abs(value - target_value) <= tolerance
        ]
        if candidates:
...
def retrieval_metrics(
    similarity: Tensor, *, k_values: Sequence[int] = (1, 5, 10)
) -> dict[str, float]:
    ...
    def direction(matrix: Tensor, prefix: str) -> dict[str, float]:
        ...
        result = {
            f"{prefix}/median_rank": float(ranks.float().median().item()),
            f"{prefix}/mrr": float((1.0 / ranks.float()).mean().item()),
        }
        for k in k_values:
            result[f"{prefix}/recall_at_{k}"] = float(
                (ranks <= min(k, count)).float().mean().item()
            )
        return result

    return {
        **direction(similarity, "audio_to_lyric"),
        **direction(similarity.transpose(0, 1), "lyric_to_audio"),
    }


def normalized_path_deviation(
    predicted_mass: Tensor, target_mass: Tensor, valid_pairs: Tensor
) -> Tensor:
    ...
    return ((predicted_mass - target_mass).abs() * valid_pairs).sum() / valid_pairs.sum().clamp_min(
        1
    )
