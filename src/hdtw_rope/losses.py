...
def cycle_consistency_loss(source_to_target: Tensor, target_to_source: Tensor, source_mask: Tensor, target_mask: Tensor) -> Tensor:
    ...
    source_index = torch.arange(source_to_target.shape[1], device=source_to_target.device).float() / max(source_to_target.shape[1] - 1, 1)
    target_index = torch.arange(target_to_source.shape[1], device=target_to_source.device).float() / max(target_to_source.shape[1] - 1, 1)
    source_error = (reconstructed_source - source_index[None, :]).abs()
    target_error = (reconstructed_target - target_index[None, :]).abs()
    return 0.5 * ((source_error * source_mask).sum() / source_mask.sum().clamp_min(1) + (target_error * target_mask).sum() / target_mask.sum().clamp_min(1))
...
class HDTWRoPELoss(nn.Module):
    ...
    def forward(self, *, similarity: Tensor, alignments: Mapping[str, AlignmentOutput], clocks: ClockOutput, anchor_targets: Mapping[str, tuple[Tensor, Tensor, Tensor]] | None = None, hierarchy_terms: Mapping[str, tuple[Tensor, Tensor, Tensor, Tensor]] | None = None, source_index_coordinate: Tensor | None = None, target_index_coordinate: Tensor | None = None) -> LossBreakdown:
        terms: dict[str, Tensor] = {"task": symmetric_contrastive_loss(similarity), "soft_dtw": soft_dtw_loss(alignments)}
        source_mono = [monotonicity_loss(clocks.source_clock[..., c], clocks.source_valid[..., c]) for c in range(clocks.source_clock.shape[-1])]
        target_mono = [monotonicity_loss(clocks.target_clock[..., c], clocks.target_valid[..., c]) for c in range(clocks.target_clock.shape[-1])]
        source_smooth = [smoothness_loss(clocks.source_clock[..., c], clocks.source_valid[..., c]) for c in range(clocks.source_clock.shape[-1])]
        target_smooth = [smoothness_loss(clocks.target_clock[..., c], clocks.target_valid[..., c]) for c in range(clocks.target_clock.shape[-1])]
        ...
        if source_index_coordinate is not None and target_index_coordinate is not None:
            source_slopes = [slope_loss(clocks.source_clock[..., c], source_index_coordinate, clocks.source_valid[..., c]) for c in range(clocks.source_clock.shape[-1])]
            target_slopes = [slope_loss(clocks.target_clock[..., c], target_index_coordinate, clocks.target_valid[..., c]) for c in range(clocks.target_clock.shape[-1])]
            terms["slope"] = torch.stack(source_slopes + target_slopes).mean()
        else:
            terms["slope"] = similarity.new_zeros(())
        terms["anchor"] = torch.stack([anchor_loss(*term) for term in anchor_targets.values()]).mean() if anchor_targets else similarity.new_zeros(())
        terms["hierarchy"] = torch.stack([containment_loss(*term) for term in hierarchy_terms.values()]).mean() if hierarchy_terms else similarity.new_zeros(())
        terms["cycle"] = cycle_consistency_loss(clocks.source_clock[..., 0], clocks.target_clock[..., 0], clocks.source_valid[..., 0], clocks.target_valid[..., 0])
        terms["entropy"] = torch.stack([alignment_entropy_loss(alignment) for alignment in alignments.values()]).mean()
