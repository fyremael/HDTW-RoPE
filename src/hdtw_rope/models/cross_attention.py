...
class HDTWRoPECrossAttention(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            or query.shape[-1] != self.model_dim
            or key_value.shape[-1] != self.model_dim
        ):
        ...
        probabilities = torch.where(
            query_mask[:, None, :, None], probabilities, torch.zeros_like(probabilities)
        )
        ...
        attended = (
            ...
            .contiguous()
            .view(query.shape[0], query.shape[1], self.model_dim)
        )
...
class BidirectionalCrossAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.audio_to_lyrics = HDTWRoPECrossAttention(
            model_dim=model_dim, num_heads=num_heads, rotary=rotary, dropout=dropout
        )
        self.lyrics_to_audio = HDTWRoPECrossAttention(
            model_dim=model_dim, num_heads=num_heads, rotary=rotary, dropout=dropout
        )
    ...
    def forward(
        ...
        audio_clock: Tensor,
        lyric_clock: Tensor,
        audio_clock_mask: Tensor,
        lyric_clock_mask: Tensor,
    ) -> tuple[Tensor, Tensor, Mapping[str, Tensor]]:
        audio_delta, audio_diag = self.audio_to_lyrics(
            self.audio_norm(audio),
            self.lyric_norm(lyrics),
            audio_mask,
            lyric_mask,
            audio_clock,
            lyric_clock,
        ...
        lyric_delta, lyric_diag = self.lyrics_to_audio(
            self.lyric_norm(lyrics),
            self.audio_norm(audio),
            lyric_mask,
            audio_mask,
        ...
        diagnostics = {
            **{f"audio_to_lyrics/{key}": value for key, value in audio_diag.items()},
            **{f"lyrics_to_audio/{key}": value for key, value in lyric_diag.items()},
        }
