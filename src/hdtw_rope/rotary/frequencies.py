...
def standard_inverse_frequencies(
    rotary_dim: int, *, base: float = 10000.0
) -> Tensor:
...
def _allocate_pairs(
    total_pairs: int, allocation: Mapping[str, float]
) -> dict[str, int]:
    ...
    floor_counts = {
        name: int(total_pairs * value) for name, value in allocation.items()
    }
    remaining = min(
...
class ClockFrequencyMap(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if mode == "fixed_log":
            ...
            omega[:, :, 0] = standard_inverse_frequencies(
                rotary_dim, base=base
            )[None, :] / (2.0 * torch.pi)
            self.register_buffer("fixed_omega", omega)
            self.register_parameter("raw_omega", None)
        elif mode == "learned_positive":
            initial = torch.full(
                (num_heads, self.num_pairs, component_count), -6.0
            )
            initial[:, :, 0] = torch.log(
                torch.expm1(
                    standard_inverse_frequencies(rotary_dim, base=base)
                    / (2.0 * torch.pi)
                ).clamp_min(1e-8)
            )[None, :]
            ...
            if partition is None or family_components is None:
                raise ValueError(
                    "clock_partitioned mode requires partition and family_components"
                )
            component_index = {
            ...
            if unknown:
                raise ValueError(
                    f"partition references unknown components: {sorted(unknown)}"
                )
            ...
            for family, count in counts.items():
                ...
                if not components:
                    raise ValueError(
                        f"family {family!r} has allocated pairs but no components"
                    )
                local_frequencies = standard_inverse_frequencies(
                    2 * count, base=base
                ) / (2.0 * torch.pi)
                for local_index in range(count):
                    component = components[local_index % len(components)]
                    omega[
                        :, cursor + local_index, component_index[component]
                    ] = local_frequencies[local_index]
                cursor += count
            self.register_buffer("fixed_omega", omega)
            self.register_parameter("raw_omega", None)
        ...
        if learn_phase_offset:
            ...
            self.register_buffer(
                "phase_offset", torch.zeros(num_heads, self.num_pairs)
            )
    ...
    def forward(
        self, clock: Tensor, clock_mask: Tensor, *, modulo: bool = True
    ) -> Tensor:
        """Return phases ``[B,H,N,P]`` in float32."""

        if (
            clock.ndim != 3
            or clock_mask.shape != clock.shape
            or clock_mask.dtype is not torch.bool
        ):
            raise ValueError(
                "clock and clock_mask must have shapes [B,N,R] with bool mask"
            )
        ...
        phases = 2.0 * torch.pi * torch.einsum(
            "bhnpr,bnr->bhnp", effective_weights, clock32
        )
